...
### 변경 내용
* 전체 6개 탭(`기본 과열 분석 & 차트`, `PRO 1`~`PRO 5`)의 **"~ 실시간 시황 분석하기" 및 "다시 분석하기" 버튼 박스 색상을 직관적인 노란색(`#FFD700`)으로 전면 변경**
* 버튼 배경이 노란색으로 강조됨에 따라 가독성 및 시인성을 극대화하기 위해 버튼 내부 텍스트 색상을 진한 검정색(`#000000`)으로 설정하고 굵기 강화를 동시 적용
* `@keyframes button-blink` 애니메이션 효과 또한 밝은 노랑(`#FFD700` ~ `#FFEE66`) 및 골드 글로우로 동기화하여 시선 집중 유도 효과 향상
